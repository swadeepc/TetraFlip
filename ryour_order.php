...
<section class="">
	...
	<section class="listings">
		<div class="wrapper">
				<h2 style="text-decoration: underline;  color: #303C6C "><b>Your Orders</b></h2>
</section>
<?php
								session_start();
								...
								$a=array();
								$i=0;
								$select = "SELECT Distinct Purchase_id as p from purchase_order where Customer_id='$cid' order by Purchase_id";
								...
								while($i>0){
									$i--;
									$select = "SELECT Resto_name  from purchase_order p,restaurant r where p.Purchase_id='$a[$i]' and p.Resto_id=r.Resto_id";
									$result = $conn->query($select);
									while($row = $result->fetch_assoc()){
...
<table align="center">
						<tr>
							<td colspan="3" style="text-align:center"><b><h2><?php echo $resto ?><h2></b></td>
						</tr>
							<tr>
								<th>Item Name</th>
								<th>Quantity</th>
					...
					window.location = (\"ryour_order.php?\")
					</script>";
								}
							 }
							}
